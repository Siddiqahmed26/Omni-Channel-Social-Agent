...
export function MobileHeader() {
    return (
        <header className="flex md:hidden items-center justify-between px-4 py-3 bg-black/40 backdrop-blur-2xl border-b border-white/5 sticky top-0 z-50 shadow-2xl">
            <div className="flex items-center gap-3">
                <div className="flex items-center justify-center p-2 rounded-xl bg-white/[0.03] border border-white/10 shadow-inner min-h-[44px] min-w-[44px]">
                    <AppSidebarTrigger isOutside={true} className="text-slate-300 hover:text-white transition-colors" />
                </div>
                <div className="scale-90 origin-left opacity-90 transition-opacity">
                    {agentInboxSvg}
                </div>
            </div>

            <div className="flex items-center gap-2">
                <div className="flex items-center gap-1 p-1 bg-white/[0.03] border border-white/10 rounded-2xl min-h-[44px]">
                    ...
                    <DropdownMenuContent align="end" className="w-56 mt-2 p-2 bg-black/60 backdrop-blur-3xl border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.5)] rounded-2xl">
                        <DropdownMenuItem asChild className="rounded-xl hover:bg-white/5 focus:bg-white/10 transition-colors p-3 cursor-pointer group">
                            <form action={logout} className="w-full">
                                <button type="submit" className="flex items-center justify-center gap-3 w-full text-red-500 font-bold text-xs uppercase tracking-widest min-h-[44px]">
